...
The tracked `.no-mistakes.yaml` sets `test.evidence.store_in_repo: true` and pins `commands.lint` to run `bin/fm-no-mistakes-preflight.sh` before `bin/fm-lint.sh`, refusing clients below the structured-attestation version floor before the repository's canonical lint gate runs.
