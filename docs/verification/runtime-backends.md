...
The real-binary result was refreshed on 2026-09-10 with Herdr 0.9.0, the portable transaction suite was verified on 2026-08-12 with Herdr 0.8.0, and the original lifecycle cases were first measured on 2026-08-02 with Herdr 0.7.5.
...
Observed bounded output, refreshed 2026-09-10 on Herdr 0.9.0 after the stale-registration fix (the two stale-registration lines are recorded under "Stale agent registration" below):
...
ok - fm-spawn --relaunch: a stale Herdr registration over a proved idle shell remains recoverable
...
The relaunch preparation treats that proved shell-only stale registration as positively agent-free, but still requires the exact endpoint to hold one idle shell before any re-rooting or launch input.
...
Backend applicability was reviewed across all five spawn adapters: tmux retains its direct wrong-worktree refusal; zellij, Orca, and cmux remain ineligible for relaunch because they still lack recovery-grade agent-state classification; only Herdr owns the idle-shell re-root primitive.
Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, OMP, Muse, Gemini, and Rovo all inherit the verified endpoint cwd before their unchanged launch templates run, so no harness-specific launch or lifecycle mechanic changed.
