...
The nested-shell result was refreshed on 2026-09-16 with Herdr 0.8.0 and Pi 0.84.2, the prior process-classifier result was measured on 2026-09-10 with Herdr 0.9.0, and the original lifecycle cases were first measured on 2026-08-02 with Herdr 0.7.5.

```sh
bin/fm-test-run.sh tests/fm-control-relaunch.test.sh
HERDR_LAB_HELPER=bin/fm-herdr-lab.sh bin/fm-test-run.sh tests/fm-control-herdr-smoke.test.sh
FM_HERDR_PI_STALE_REGISTRATION_LIVE_E2E=1 HERDR_LAB_HELPER=bin/fm-herdr-lab.sh bin/fm-test-run.sh tests/fm-herdr-pi-stale-registration-live-e2e.test.sh
```

Observed bounded output on 2026-09-16:
...
ok - fm-spawn --relaunch: liveness and nested foreground-shell identity races plus failed cwd postconditions refuse replacement launch
...
ok - real herdr: an unregistered endpoint with a foreground non-shell process receives no reroot or replacement-launch input
ok - real herdr 0.8.0: a registration Herdr keeps after its agent exits reads stale-agent and recovers as dead
ok - real herdr: a stale registration no longer blocks relaunch, and the endpoint and local copy survive
ok - real herdr: landed work in a nested-shell endpoint closes the exact pane and returns its isolated worktree
ok - real herdr: unlanded work refuses cleanup before the nested endpoint or isolated worktree is touched
ok - real herdr 0.8.0 + pi 0.84.2: a completed Pi relaunches from Treehouse's nested idle shell in the same endpoint and isolated worktree
```

The registry read through `herdr pane report-agent` is the same source `fm_backend_herdr_agent_state` classifies, and that registration counts as an agent only while `pane process-info` shows a harness process behind it, so the smoke backs the registration with a real process named like a harness and then stops that process.
The relaunch preparation treats a process-proven shell-only stale registration as positively agent-free, then requires one settled foreground process group containing one recognized sleeping or idle shell whose ancestry reaches the pane's outer shell and no supported harness process anywhere below that outer shell.
That proof accepts the nested worktree shell left by `treehouse get` without requiring its pid to equal the outer pane-shell pid, while a live or unregistered harness, foreground non-shell command, unreadable ancestry, multiple or unsettled foreground processes, or changed foreground-shell identity refuses before the next input.
...
Worker-tool applicability was reviewed across Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, OMP, Muse, Gemini, Rovo, Agy, and Devin.
Every supported tool reaches the same Herdr preparation and identity revalidation immediately before each input in its unchanged launch template, while Muse, Gemini, Rovo, Agy, and Devin remain inapplicable only to secondmate relaunch because their existing adapter contract supports crewmates and scouts only.
...
The same endpoint recovery path was refreshed live on 2026-09-16 against Herdr 0.8.0 protocol 19, while the 0.9.0 protocol 22 result above remains the newer-release evidence.
...
```text
ok - real herdr 0.8.0: a gone session reads recoverable while a live pane and a malformed target do not
ok - real herdr: a drifted agent-free shell returns to its worktree and reuses the same endpoint
ok - real herdr: an unregistered endpoint with a foreground non-shell process receives no reroot or replacement-launch input
```
...
This proves subcommand presence in the client only, while the server response shape is measured on 0.8.0 and 0.9.0 above.
...
It now carries the completed Pi through a same-endpoint nested-shell relaunch, so the classifier and relaunch halves cannot pass separately while their composition is broken:
...
```text
# pi 0.84.2 under herdr 0.8.0: registered idle, foreground [{"name":"node","argv0":"pi"}]
ok - real herdr 0.8.0 + pi 0.84.2: a running registered pi classifies alive at process level
# herdr 0.8.0 kept the pi registration (idle) after /quit under a nested shell: the stale-registration branch is exercised
ok - real herdr 0.8.0 + pi 0.84.2: the registration left behind by a quit pi reads stale-agent and recovers as dead
ok - real herdr 0.8.0 + pi 0.84.2: a completed Pi relaunches from Treehouse's nested idle shell in the same endpoint and isolated worktree
```
