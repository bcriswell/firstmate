...
#   merely unreachable from here, and only Herdr can prove the endpoint gone by
#   reading the recorded pane again after its exact session server is running.
#   A tmux `missing` always refuses because a task record carries no socket
#   identity with which to distinguish a destroyed window from an unreachable
#   one, and an endpoint that turns out to have survived refuses too. The
#   worktree is reused untouched either way, and only a crewmate or scout may
#   rebind because a secondmate's owner retains its respawn path. Every adopted
#   or re-created Herdr endpoint must prove one idle foreground shell attributable
#   to the outer pane shell, including Treehouse's nested shell. A drifted shell
#   is re-rooted, an already-rooted shell is unchanged, every replacement-launch
#   input is preceded by identity revalidation, and every ambiguous endpoint or
#   wrong-cwd endpoint on another backend refuses.
...
# A secondmate relaunch adopts a home rather than a project task worktree, so it
# cannot use spawn_worktree_isolated. It must still prove that its recorded home
# is exactly a readable Git worktree root before any endpoint mutation.
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  # agent-free, holds one settled idle foreground shell attributable through
  # the outer pane shell, and has no supported harness descendant before
  # deciding whether it needs to recover a drifted
  # endpoint; an already-rooted idle endpoint is accepted unchanged. Recovery
  # clears unsubmitted shell input, runs an injection-safe cd, and verifies both
  # cwd and foreground-shell identity before this launch continues.
  # Every ambiguity and every other backend's wrong cwd retains the existing
  # refusal.
  if [ "$KIND" = secondmate ]; then
  ...
  RELAUNCH_STATE=$(fm_backend_agent_state "$BACKEND" "$WT_TARGET")
  [ "$RELAUNCH_STATE" = dead ] || {
