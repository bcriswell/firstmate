...
#   or herdr). Every Herdr relaunch requires one proved idle foreground shell
#   attributable to the outer pane shell, including Treehouse's nested shell; a
#   drifted endpoint is then re-rooted, an already-rooted endpoint is unchanged,
#   and every other wrong-cwd or ambiguous endpoint refuses. The previous harness's
